...
class plgAuthenticationSocialConnectFacebook extends JPlugin
{
	...
	function onAuthenticate($credentials, $options, &$response)
	{
		...
		{
      $params = version_compare(JVERSION, '1.6.0', 'ge') ? $application->getParams('com_socialconnect') : JComponentHelper::getParams('com_socialconnect');
			$parameters = array('access_token' => $socialConnectFacebookAccessToken, 'fields' => 'id,name,email', 'appsecret_proof' => hash_hmac('sha256', $socialConnectFacebookAccessToken, $params->get('facebookApplicationSecret')));
			JLoader::register('SocialConnectHelper', JPATH_SITE.'/components/com_socialconnect/helpers/socialconnect.php');
			$result = SocialConnectHelper::request('https://graph.facebook.com/v2.5/me', $parameters, 'GET');
			if (extension_loaded('json'))
			{
				$data = json_decode($result);
			}
			$data->image = 'https://graph.facebook.com/v2.5/'.$data->id.'/picture';
			if (isset($data->about))
			{
