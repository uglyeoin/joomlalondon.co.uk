...
class Replace
{
	...
	private static function getSetItem($match, &$set_ids, $only_basic_details = false)
	{
		...
		$item->id       = isset(self::$sets[$item->set]) ? count(self::$sets[$item->set]) : 0;
	...
	private static function removeByAccess(&$string)
	{
		...
		foreach ($remove_sets as $set_id)
		{
			...
			$regex = RL_RegEx::quote($first_set_item->orig)
				. '.*?'
		...
		foreach ($remove_items as $item)
		{
			...
			$regex = RL_RegEx::quote($item->orig)
				. '.*?'
	...
	private static function replaceSyntaxItem(&$string, $item, $items, $first = 0)
	{
		...
		{
			$class = 'anchor';
			$class .= ' rl_tabs-sm-scroll nn_tabs-sm-scroll';
	...
	private static function getPreHtml($item, $items, $first = 0)
	{
		...
		if (Params::get()->use_responsive_view)
		{
			$class .= ' rl_tabs-responsive nn_tabs-responsive';

			$html[] = '<div class="rl_tabs-responsive nn_tabs-responsive">';
			$html[] = self::getResponsiveNav($items);
		}
	...
	private static function getMainClasses($item)
	{
		...
		$position = isset($item->position) ? $item->position : Params::getPositioning();
	...
	private static function getNav(&$items)
	{
		...
		foreach ($items as $item)
		{
			...
			$class .= $item->scroll ? ' rl_tabs-doscroll nn_tabs-doscroll' : '';
	...
	private static function getResponsiveNav(&$items)
	{
		...
		// Nav for mobile view
		$html[] = '<ul class="nav nav-tabs nav-stacked rl_tabs-sm nn-tabs-sm" id="set-rl_tabs-sm-' . $items[0]->set . '">';
		foreach ($items as $item)
		{
			$html[] = '<li class="' . self::getItemClass($item, 'rl_tabs-tab-sm nn_tabs-tab-sm') . '">';
			...
			$html[] = '<a href="#' . $item->id . '" class="rl_tabs-toggle-sm nn_tabs-toggle-sm">'
				. '<span class="rl_tabs-sm-inner nn_tabs-sm-inner">'
				. $item->title_full
				. '</span></a>';
			$html[] = '</li>';
		}
