...
class Items
{
	...
	private static function getOutput($item, $content_item)
	{
		...
		$data_tags = ($item->type == 'k2') ? new DataTagsK2 : new DataTags;
	...
	private static function prepareContentItem($item, $content_item, $output)
	{
		...
		if ($item->type == 'k2')
		{
			$file = 'media/k2/items/cache/' . md5("Image" . $content_item->id) . '_L.jpg';
			if (JFile::exists(JPATH_SITE . '/' . $file))
			{
				$content_item->image_url = JUri::root() . $file;

				$content_item->image = DataTags::getImageHtml($content_item->image_url, $content_item->title, $content_item->image_caption, 'k2_image', false);
			}

			$file = 'media/k2/items/cache/' . md5("Image" . $content_item->id) . '_S.jpg';
			if (JFile::exists(JPATH_SITE . '/' . $file))
			{
				$content_item->thumb_url = JUri::root() . $file;

				$content_item->thumb = DataTags::getImageHtml($content_item->thumb_url, $content_item->title, $content_item->image_caption, 'k2_image k2_thumb', false);
			}
		}

		if (strpos($output, 'tag') !== false)
		{
			$method = ($item->type == 'k2') ? 'addTagsK2' : 'addTags';
	...
	private static function getContentItemsBySetType($data)
	{
		...
		if (isset($data->featured) && empty($data->ids) && empty($data->categories))
		{
			return self::getFeaturedItems($data);
		}

		if ( ! empty($data->is_category))
		{
			return self::getCategoryItems($data);
		}

		if ( ! empty($data->is_tag))
		{
			return self::getTagItems($data);
		}
	...
	private static function storeSingleContentItemsFromDatabase($database_ids)
	{
		...
		foreach ($database_ids as $type => $ids)
		{
			...
			$ordering = self::getOrdering($item);
	...
	public static function getQueryIdLists($ids, $type = 'article')
	{
		...
		foreach ($ids as $id)
		{
			...
			{
				case 'category':
					$catid = Article::get('catid') ?: Article::get('id');

					$input = JFactory::getApplication()->input;

					if ( ! $catid && $input->get('option') == 'com_content' && $input->get('view', 'category') == 'category')
					{
						$catid = $input->get('id');
					}
	...
	private static function getCategoryItems($item)
	{
		...
		$cat_ids = self::getCategoryIds($item);
		...
		if ( ! empty($item->id))
		{
			self::setWhereConditionsForContentIds($query, $item->id);
		}
	...
	private static function getCategoryIds($item)
	{
		list($ids, $titles, $likes) = self::getQueryIdLists($item->categories, 'category');
		...
		if ( ! empty($titles))
		{
			$wheres[] = $db->quoteName('c.' . self::$cat_title) . ' IN (' . implode(',', $titles) . ')';
			$wheres[] = $db->quoteName('c.' . self::$cat_alias) . ' IN (' . implode(',', $titles) . ')';
		...
		if ( ! empty($likes))
		{
			...
			{
				$wheres[] = $db->quoteName('c.' . self::$cat_title) . ' LIKE ' . $like;
				$wheres[] = $db->quoteName('c.' . self::$cat_alias) . ' LIKE ' . $like;
			}
			$query->where('(' . implode(' OR ', $wheres) . ')');
		}

		list($ignore_language, $ignore_state, $ignore_access) = self::getIgnores($item, 'categories');

		if ( ! $ignore_language)
		{
			$query->where($db->quoteName('c.language') . ' IN (' . $db->quote(JFactory::getLanguage()->getTag()) . ',' . $db->quote('*') . ')');
		}

		if ( ! $ignore_state)
		{
			$where = ($item->type == 'k2')
				? $db->quoteName('c.published') . ' = 1 AND ' . $db->quoteName('c.trash') . ' = 0'
		...
		if ( ! $ignore_access)
		{
			$query->where($db->quoteName('c.access') . ' IN (' . implode(', ', Params::getAuthorisedViewLevels()) . ')');
		}
	...
	private static function getTagItemIds($item, $is_category = false)
	{
		...
		if ($item->type == 'k2')
		{
			...
			$query = $db->getQuery(true)
				...
				->where($db->quoteName('tagID') . ' IN (' . implode(',', $tag_ids) . ')');
			$db->setQuery($query);
	...
	private static function getSingleContentItems($item)
	{
		...
		foreach ($item->ids as $id)
		{
			...
			if (strpos($id, '*') !== false)
			{
				$found_items = self::getSingleContentItemByWildcard($item, $id);
	...
	private static function getSingleContentItemByWildcard($item, $id)
	{
		$db = JFactory::getDbo();
	...
	private static function getSingleContentItemStored($item, $id)
	{
		...
		if ($featured && empty($stored->featured))
		{
			return false;
		}
		...
		if ( ! $ignore_state)
		{
			$state = ($item->type == 'k2') ? 'published' : 'state';
	...
	private static function setItemQueryConditions(JDatabaseQuery &$query, $item)
	{
		...
		{
			$query->where($db->quoteName('a.featured') . ' = ' . (int) $item->featured);
		}
		...
		if ( ! $ignore_state)
		{
			...
			$where = ($item->type == 'k2')
				? $db->quoteName('a.published') . ' = 1 AND ' . $db->quoteName('a.trash') . ' = 0'
	...
	private static function getOrdering($item = null)
	{
		...
		$ordering           = isset($item->ordering) ? $item->ordering : $params->ordering;
		$ordering_direction = isset($item->ordering_direction) ? $item->ordering_direction : $params->ordering_direction;
		...
		$ordering = self::parseOrdering($ordering);

		return JFactory::getDbo()->quoteName($ordering) . ' ' . $ordering_direction;

	}

	private static function parseOrdering($ordering)
	{
		switch ($ordering)
		{
			case 'cat':
			case 'cat_title':
			case 'cat_name':
				return 'c.' . self::$cat_title;

			case 'cat_alias':
				return 'c.' . self::$cat_alias;
	...
	private static function getTagValues($data)
	{
		...
		foreach ($parts as $string)
		{
			...
			$type = (isset($set->type) && $set->type == 'k2') ? 'k2' : 'joomla';
			$set->type = $type;

			$set->ids = [];

			if ( ! empty($set->limit) && RL_RegEx::match('^([0-9]+)-([0-9]+)$', $set->limit, $limit))
			{
				$set->limit  = $limit['2'] - $limit['1'] + 1;
				$set->offset = $limit['1'] - 1;
			}

			if ( ! empty($set->tag) || ! empty($set->is_tag))
			{
				$set->tags   = self::getIdsFromString(empty($set->tag) ? $set->id : $set->tag);
				$set->is_tag = true;
			}

			if ( ! empty($set->category) || ! empty($set->is_category))
			{
				$set->categories  = self::getIdsFromString(empty($set->category) ? $set->id : $set->category);
				$set->is_category = true;
			}

			if (isset($set->featured) && empty($set->is_category) && empty($set->is_tag))
			{
				$sets[] = $set;
				continue;
			}
			...
			$set->ids = $data['tag'] == $params->articles_tag ? self::getIdsFromString($set->id) : [$set->id];
