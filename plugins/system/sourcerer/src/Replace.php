...
class Replace
{
	...
	private static function handleMatch(&$match, $area = 'article', $article = '', $remove = false)
	{
		...
		// Load the css file if css=... is used in the {source} tag
		$file = ! empty($data->css)
			? $data->css
			: (! empty($data->style)
				? $data->style
				: (! empty($data->stylesheet)
					? $data->stylesheet
					: ''
				)
			);
		...
		$file = ! empty($data->{'include_once'}) ? $data->{'include_once'} : '';
		if ( ! empty($file) && JFile::exists(JPATH_SITE . $params->include_path . $file))
		{
			$content .= '<?php include_once JPATH_SITE . \'' . $params->include_path . $file . '\'; ?>';
		}

		// Add the php file if require=... is used in the {source} tag
		$file = ! empty($data->{'require'}) ? $data->{'require'} : '';
		if ( ! empty($file))
		{
			$content .= '<?php require JPATH_SITE . \'' . $params->include_path . $file . '\'; ?>';
		}

		// Add the php file if require_once=... is used in the {source} tag
		$file = ! empty($data->{'require_once'}) ? $data->{'require_once'} : '';
		if ( ! empty($file))
		{
			$content .= '<?php require_once JPATH_SITE . \'' . $params->include_path . $file . '\'; ?>';
		}
	...
	private static function replaceTagsByType(&$string, $area = 'article', $type = 'all', $article = '')
	{
		...
		$security_pass = isset($a->{'security_pass' . $type_ext}) ? $a->{'security_pass' . $type_ext} : true;
