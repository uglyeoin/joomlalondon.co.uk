...
class plgSystemSocialConnect extends JPlugin
{
	...
	public function onAfterRoute()
	{
		...
		if ($application->isSite() && $view == 'login' && ($option == 'com_users' || $option == 'com_user') && $_SERVER['REQUEST_METHOD'] == 'GET' && $task != 'registration.activate')
		{
			...
			if (count($items))
			{
				...
				$ssl = $items[0]->params->get('secure');
			}
			else
			{
				$link = 'index.php?option=com_socialconnect&view=login';
			}
			$redirect = JRoute::_($link, false, (int)$ssl);
			if ($return)
			{
				if (strpos($redirect, '?'))
				{
		...
		if ($document->getType() == 'html')
		{
			...
			else
			{
				if ($this->isAutoPostSupported())
				{
					...
					$css = (version_compare(JVERSION, '2.5', 'lt')) ? '/plugins/content/socialconnectautopost/includes/tmpl/'.$layout.'/css/style.css?v=1.8.0' : '/plugins/content/socialconnectautopost/socialconnectautopost/includes/tmpl/'.$layout.'/css/style.css?v=1.8.0';
					...
					$script = (version_compare(JVERSION, '2.5', 'lt')) ? '/plugins/content/socialconnectautopost/includes/js/script.js?v=1.8.0' : '/plugins/content/socialconnectautopost/socialconnectautopost/includes/js/script.js?v=1.8.0';
	...
	private function addAutoPost()
	{
		...
		if ($this->isAutoPostSupported() && $document->getType() == 'html')
		{
			...
			$linkedin = $params->get('linkedinAutoPost') && $params->get('linkedinAutoPostDataToken');
	...
	private function isAutoPostSupported()
	{
		...
		$twitter = $params->get('twitterAutoPost') && $params->get('twitterAutoPostDataToken') && $params->get('twitterConsumerKey') && $params->get('twitterConsumerSecret');
		$linkedin = $params->get('linkedinAutoPost') && $params->get('linkedinAutoPostDataToken');
		return in_array($context, $extensions) && ($facebook || $twitter || $linkedin);
	}
