...
class plgContentSocialConnectAutoPost extends JPlugin
{
		...
		if (is_array($services) && count($services))
		{
			...
			if (isset($url))
			{
				...
				foreach ($services as $service)
				{
					...
					{
						$this->postToLinkedIn($title, $url, $suffix);
					}
				}
			}
		}
	...
	private function postToFacebook($title, $url, $suffix)
	{
		...
		if (version_compare(JVERSION, '2.5', 'ge'))
		{
			...
			// Search for a page token
			$availableAccounts = json_decode($data->availableAccounts);
			if (is_null($availableAccounts))
			{
				$availableAccounts = json_decode(html_entity_decode($data->availableAccounts));
			}
			foreach ($availableAccounts as $account)
			{
				if ($account->id == $data->account && isset($account->token))
				{
					$params->set('facebookAutoPostDataToken', $account->token);
				}
			}
		}
		...
		if ($token && $account)
		{
			...
			$result = SocialConnectHelper::request('https://graph.facebook.com/v2.2/'.$account.'/feed', $parameters, 'post');
			if ($result)
			{
	...
	private function postToLinkedIn($title, $url, $suffix)
	{
		$params = JComponentHelper::getParams('com_socialconnect');
		if (version_compare(JVERSION, '2.5', 'ge'))
		{
			$data = $params->get('linkedinAutoPostData');
			if (is_object($data) && isset($data->token))
			{
				$params->set('linkedinAutoPostDataToken', $data->token);
			}
		}

		$token = $params->get('linkedinAutoPostDataToken');

		if ($token)
		{
			require_once JPATH_SITE.'/components/com_socialconnect/helpers/socialconnect.php';
			$payload = new stdClass;
			$payload->comment = $title.' '.$suffix.' '.$url;
			$payload->visibility = new stdClass;
			$payload->visibility->code = 'anyone';
			$json = json_encode($payload);
			$parameters = array();
			$options = array(CURLOPT_HTTPHEADER => array('Authorization: Bearer '.$token, 'Content-Type: application/json', 'x-li-format: json'), CURLOPT_POSTFIELDS => $json);
			$result = SocialConnectHelper::request('https://api.linkedin.com/v1/people/~/shares?format=json', $parameters, 'post', $options, false, true);
			if ($result)
			{
				$response = json_decode($result);
				...
				{
					$application = JFactory::getApplication();
					$application->enqueueMessage(JText::sprintf('JW_SC_AUTOPOST_ERROR', 'LinkedIn', $response->message), 'notice');
				}
			}
		}
	}
