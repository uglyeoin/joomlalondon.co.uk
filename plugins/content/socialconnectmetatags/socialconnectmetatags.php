...
class plgContentSocialConnectMetaTags extends JPlugin
{
	...
	// Joomla! 1.5 event
	public function onPrepareContent($row, $params, $offset = 0)
	{
	...
	private function addHeadData($context, $row, $params, $offset)
	{
		...
		switch($context)
		{
			...
			case 'com_k2.item' :
				...
				if (isset($row->$facebookImage) && $row->$facebookImage)
				{
					$parts = parse_url($row->$facebookImage);
					$image = substr(JURI::root(), 0, -1).str_replace(JURI::root(true), '', $row->$facebookImage);
		...
		// Add URL meta tag for Joomla! versions that do not include a canonical link
