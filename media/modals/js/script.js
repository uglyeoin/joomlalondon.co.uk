...
(function($) {
	...
	RegularLabsModals = {
		...
		init: function(options) {
			...
			// Fix layout issues caused by navigation buttons by temporarily hiding them
			$(document).bind('rl_modals_complete', function() {
				$('#rl_modals_content > button').each(function() {
					var el      = $(this);
		...
		initModal: function(el) {
			...
			defaults['onComplete'] = function() {
				...
				if (defaults['autoclose'] != undefined && defaults['autoclose']) {
					...
					$('#rl_modals_title .countdown').animate({
						width: 0
					}, time, 'linear');
			...
			/* Disable rl_modals on mobile devices */
