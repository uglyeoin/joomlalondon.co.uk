...
(function($, undefined) {
	...
	var Datepicker = function(element, options) {
		...
		if (this.o.calendarWeeks){this.picker.find('tfoot th.today')
	...
	Datepicker.prototype = {
		...
		update: function(){
			...
			if (fromArgs) {
				...
				if (oldDate && this.date && oldDate.getTime() !== this.date.getTime()){this._trigger('changeDate');}
			}
			if (!this.date && oldDate){this._trigger('clearDate');}
		...
		setRange: function(range){
			if (!range || !range.length){delete this.range;} else {this.range = $.map(range, function(d){ return d.valueOf(); });}
			this.fill();
		},
		...
		_setDate: function(date, which){
			if (!which || (which == 'date')){this.date = date && new Date(date);}
