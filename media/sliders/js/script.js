	...
	RegularLabsSliders = {
		...
		showByCookies: function() {
			...
			for (var i = 0; i < cookies.length; i++) {
				...
				if (key.substr(0, 14) != 'set-rl_sliders') {
					continue;
				}
		...
		getScrollToElement: function($el) {
			var scrolling_type = $el.closest('.rl_sliders-group').find('.rl_sliders-toggle').hasClass('rl_sliders-item-scroll')
				? 2
				: this.options.scroll;
			...
			switch (scrolling_type) {
				case 2:
					// Scroll to slider
					var $scroll_to = $el.closest('.rl_sliders-group').find('.rl_sliders-scroll').first();
				...
				default:
					...
					var $scroll_to = $el.closest('.rl_sliders').find('.rl_sliders-scroll').first();
					break;
			}
		...
		initHashLink: function(el) {
			...
			$link.click(function(e) {
				...
				var slider_open = $slider.hasClass('in');
				var scroll_to   = is_slider ? self.getScrollToElement($slider) : $anchor;

				self.showByID(slider_id);
				if (scroll) {
					// Scroll if slider is already open
					if (slider_open) {
						$('html,body').animate({scrollTop: scroll_to.offset().top});
						history.replaceState({}, '', self.current_url + '#' + id);
						return;
					}
					// Slider is closed, so scroll when slider is opened
					$slider.one('shown.bs.collapse', function(e) {
						$('html,body').animate({scrollTop: scroll_to.offset().top});
						history.replaceState({}, '', self.current_url + '#' + id);
						e.stopPropagation();
					});
		...
		initHoverMode: function() {
			...
			$('body').on('mouseenter.collapse.data-api', 'a.rl_sliders-toggle', function(e) {
				var group = $(this).closest('.rl_sliders-group');

				if (
					(mode == 'hover' && group.hasClass('click'))
					|| (mode == 'click' && !group.hasClass('hover'))
				) {
					return;
				}

				e.preventDefault();
				RegularLabsSliders.show($(this).attr('data-id'));
			});

			$('body').on('click.collapse.data-api', 'a.rl_sliders-toggle', function(e) {
				var group = $(this).closest('.rl_sliders-group');

				if (!group.hasClass('hover')) {
					return;
				}

				group.removeClass('hover');

				$('body').one('mouseenter.collapsed.data-api', 'a.rl_sliders-toggle', function(e) {
		...
		initCookieHandling: function() {
			...
			$('.rl_sliders-body').on('show.bs.collapse', function(e) {
				self.addCookie($(this).attr('id'));

			});
			$('.rl_sliders-body').on('hidden.bs.collapse', function(e) {
				self.removeCookie($(this).attr('id'));
			});
		},

		addCookie: function(id) {
			var set = this.getSetId(id);

			var cookies = this.getCookiesBySet(set);

			cookies['set-rl_sliders-' + set] = id;

			this.setCookiesBySet(set, cookies);
		},
		...
		removeCookie: function(id) {
			...
			delete cookies['set-rl_sliders-' + set];

			this.setCookiesBySet(set, cookies);
		},

		getCookiesBySet: function(set) {
			var obj     = {};
			var cookies = $.cookie(this.options.cookie_name);
		...
		setCookiesBySet: function(set, cookies) {
			var arr = [];
			for (var set in cookies) {
				if (set && cookies[set]) {
					arr[arr.length] = set + '=' + cookies[set];
				}
			}

			$.cookie(this.options.cookie_name, arr.join('___'));
		},

		getSetId: function(id) {
			var set = 0;

			$('#' + id).closest('.rl_sliders').each(function($i, el) {
				set = el.id;
				return;
			});
