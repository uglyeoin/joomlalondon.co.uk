...
(function($) {
	...
	RegularLabsSourcererPopup = {
		...
		insertText: function() {
			...
			var css_code = Joomla.editors.instances['css_code'].getValue().trim();
			if (css_code) {
				if (css_code.hasLineBreaks()) {
					css_code = css_code.indent();
				}

				pre_php.push('$doc->addStyleDeclaration("' + css_code.escapeQuotes() + '");');
			}

			var js_code = Joomla.editors.instances['js_code'].getValue().trim();
			if (js_code) {
				if (js_code.hasLineBreaks()) {
					js_code = js_code.indent();
				}

				if ($form['js_code_placement'].value == 'inline') {
					codes.push('<script>' + js_code + '</script>');
				} else {
					pre_php.push('$doc->addScriptDeclaration("' + js_code.escapeQuotes() + '");');
				}
			}

			var php_code = Joomla.editors.instances['php_code'].getValue().trim();
			if (php_code) {
			...
			if ($form['php_file'].value.trim()) {
				attributes.push($form['php_include_method'].value.trim() + '="' + $form['php_file'].value.trim() + '"');
			}

			if ($form['css_file'].value.trim()) {
				attributes.push('css="' + $form['css_file'].value.trim() + '"');
			}

			if ($form['js_file'].value.trim()) {
				attributes.push('js="' + $form['js_file'].value.trim() + '"');
			}
