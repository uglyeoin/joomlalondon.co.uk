...
(function($) {
	if (typeof( window['arkScripts'] ) == "undefined") {
		...
		arkScripts = {
			loadajax: function(url, succes, fail, query, timeout, dataType) {
				if (url.substr(0, 9) != 'index.php') {
					...
					url = 'index.php?ark_qp=1&url=' + escape(url);
					if (timeout) {
						url += '&timeout=' + timeout;
					}
				}
			...
			displayVersion: function(data, extension, version, is_pro) {
				...
				var compare = arkScripts.compareVersions(version, new_version);
			...
			initCheckAlls: function(id, classname) {
				$('#' + id).attr('checked', arkScripts.allChecked(classname));
			...
			checkAll: function(checkbox, classname) {
				var allchecked = arkScripts.allChecked(classname);
	...
	$(document).ready().delay(1000, function() {
		$('.btn-group.ark_btn-group label').click(function() {
			var label = $(this);
		...
		$('.btn-group.ark_btn-group input[checked=checked]').each(function() {
