...
(function($) {
	...
	RegularLabsTabsPopup = {
		...
		selects : ['access', 'usergroup'],
		radio   : ['positioning', 'alignment'],
		...
		insertText: function() {
			...
			$('.tab-pane').each(function($i, el) {
				...
				var slideshow_radio = $('input[name="tab_1[slideshow_radio]"]:checked').val();
				var slideshow       = $('input[name="tab_1[slideshow]"]').val();

				if (slideshow_radio < 1) {
					slideshow = '';
				} else {
					slideshow = slideshow ? slideshow : 1;
				}

				form['tab_1_slideshow'].value = slideshow;
				...
				$.each(self.selects, function(s, name) {
					var field_name = el.id + '[' + name + ']';

					var select_field = $('select[name="' + field_name + '"]');

					if (!select_field.attr('value')) {
						return;
					}

					var option_field = $('select[name="' + field_name + '"] option[value="' + select_field.attr('value') + '"]');

					var option_name = self.toTextValue(option_field.html());

					parameters[name] = option_name;
				});
				...
				if (form[el.id + '[extra]'].value) {
					params.push(form[el.id + '[extra]'].value);
				}
