...
var demo = (function(window, undefined) {
  ...
  function _mapPolygons(pattern) {
    ...
    jQuery(SELECTORS.pattern).append(pattern);
    ...
    polygonMap.paths.forEach(function(polygon) {
      ...
      jQuery(polygon).attr('class', CLASSES.polygon + ' ' + CLASSES.polygonHidden);
    ...
    jQuery(SELECTORS.pattern).removeClass(CLASSES.patternHidden);
  };
  ...
  function _bindCards() {
    ...
    var elements = jQuery(SELECTORS.card);

    jQuery.each(elements, function(card, i) {
      ...
      var cardImage = jQuery(card).find(SELECTORS.cardImage);
      var cardClose = jQuery(card).find(SELECTORS.cardClose);

      jQuery(cardImage).on('click', _playSequence.bind(this, true, i));
      jQuery(cardClose).on('click', _playSequence.bind(this, false, i));
    });
  };
  ...
  function _onCardMove(track) {
    ...
    polygonMap.points.forEach(function(point, i) {
      ...
      if (_detectPointInCircle(point, radius, center)) {
        jQuery(polygonMap.paths[i]).attr('class', CLASSES.polygon);
      } else {
        jQuery(polygonMap.paths[i]).attr('class', CLASSES.polygon + ' ' + CLASSES.polygonHidden);
      }
    });
  }
