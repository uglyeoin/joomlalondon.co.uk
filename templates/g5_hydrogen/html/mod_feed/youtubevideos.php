...
else
{
	...
	{ ?>
		...
		<?php for ($i = 0, $max = min(count($feed), $params->get('rssitems', 3)); $i < $max; $i++) { ?>
			<?php
				$date 	= isset($feed[i]->updatedDate->date) ? $feed->updatedDate->date : null;

				echo "URL: " . $uri . ", Title: " . $title . ", Date: " . $date;
			...
			?>
				<li>
					...
					<?php if ($params->get('rssitemdate', 0)) : ?>
						...
						<div><?php echo JHtml::_('date', $feed[$i]->updatedDate->date, JText::_('DATE_FORMAT_LC3')); ?>
					<?php endif; ?>
