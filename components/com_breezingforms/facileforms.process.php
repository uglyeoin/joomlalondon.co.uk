            ...
            foreach($list_ids As $list_id){
                ...
                if ($email != '' && $checked) {
                    $try = $api->put('lists/' . trim($list_id) . '/members/' . $subHash, [
                        'email_address'     => $email,
                        'merge_fields'      => (count($mergeVars) == 0 ? '' : $mergeVars),
		...
		if (!$halt) {
			...
			if($this->formrow->double_opt){
				...
				if(bf_is_email($recipient))
				{
					...
					$database = JFactory::getDbo();
					$database->setQuery( "UPDATE #__facileforms_records SET opt_token=" . $database->quote( base64_encode( $token ) ) . " WHERE id=" . $database->quote( $lastID ) );
					$database->execute();

					$opt_in_link = $domainAddress . '?option=com_breezingforms&opt_in=true&id=' . $lastID . '&' . 'token=' . base64_encode( $token );
					$opt_out_link = $domainAddress . '?option=com_breezingforms&opt_out=true&id=' . $lastID . '&' . 'token=' . base64_encode( $token );
...
.thebutton {
    ...
    transition: box-shadow .15s linear;
								...
								if(isset($options['emailfield']) && $options['emailfield'] !== '') {
									$stripeemail = strtolower(JRequest::getVar('ff_nm_'.$options['emailfield'], '')[0]);
