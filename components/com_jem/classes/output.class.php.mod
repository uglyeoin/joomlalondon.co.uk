...
class JEMOutput
{
	...
	static function printbutton($print_link, $params)
	{
		...
		if ($settings->get('global_show_print_icon',0)) {
			...
			if ($app->input->get('print','','int')) {
				...
				$output = '<a class="btn" href="#" onclick="window.print();return false;">'.$pimage.'</a>';
			} else {
				//button in view
				$overlib = JText::_('COM_JEM_PRINT_DESC');
				$text = JText::_('COM_JEM_PRINT');
				$output	= '<a class="btn" href="'. JRoute::_($print_link) .'" '.JEMOutput::tooltip($text, $overlib, 'editlinktip', 'bottom')
	 ...
	 */
	static function tooltip($title, $text, $classes = 'btn', $position = '')
	{
		$result = array();
