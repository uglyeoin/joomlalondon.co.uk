...
class SocialConnectOAuthHelper
{
	...
	public static function authorize()
	{
		...
		if (empty(self::$code))
		{
			...
			$parameters = array('client_id' => self::$client_id, 'redirect_uri' => self::$redirect_uri, 'state' => self::$state, 'scope' => self::$scope, 'display' => self::$display, 'response_type' => 'code');
		...
		{
			$parameters = array('code' => self::$code, 'client_id' => self::$client_id, 'client_secret' => self::$client_secret, 'redirect_uri' => self::$redirect_uri, 'grant_type' => 'authorization_code');
	...
	public static function legacyAuthorize()
	{
		...
		if (!$oauthToken)
		{
			$tmhOAuth = new tmhOAuth( array('consumer_key' => self::$consumer_key, 'consumer_secret' => self::$consumer_secret));
			$request_token_endpoint = self::$service == 'Yahoo' ? 'get_request_token' : 'request_token';
			$tmhOAuth->request('POST', self::$endpoint.$request_token_endpoint, array('oauth_callback' => self::$oauth_callback));
			if ($tmhOAuth->response['code'] == 200)
			{
				...
				if ($data['oauth_callback_confirmed'] == true)
				{
					...
					if(self::$service == 'Yahoo') {
						$authenticate_endpoint = 'request_auth';
					} else if(self::$service == 'Tumblr') {
						$authenticate_endpoint = 'authorize';
					}
					$application->redirect(self::$endpoint.$authenticate_endpoint.'?oauth_token='.$data['oauth_token']);
				}
			}
			else
			{
		...
		else if ($oauthVerifier)
		{
			...
			{
				$tmhOAuth = new tmhOAuth( array('consumer_key' => self::$consumer_key, 'consumer_secret' => self::$consumer_secret, 'user_token' => $socialConnectOAuthUserToken, 'user_secret' => $socialConnectOAuthUserSecret));
				$access_token_endpoint = self::$service == 'Yahoo' ? 'get_token' : 'access_token';
				$tmhOAuth->request('POST', self::$endpoint.$access_token_endpoint, array('oauth_verifier' => $oauthVerifier));
				if ($tmhOAuth->response['code'] == 200)
				{
					$data = $tmhOAuth->extract_params($tmhOAuth->response['response']);
